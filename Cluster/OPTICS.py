...
# Tạo dữ liệu mẫu với tọa độ latitude và longitude
data = np.array(
    [
...
# Sử dụng OPTICS để phân cụm dữ liệu
clustering = OPTICS(min_samples=8).fit(data)

# Hiển thị kết quả trên biểu đồ Matplotlib
...
plt.figure(figsize=(10, 6))
colors = ["c.", "b.", "r.", "g.", "m.", "y."]
for class_label in range(max(labels) + 1):
    plt.plot(
        data[labels == class_label][:, 0],
        data[labels == class_label][:, 1],
        colors[class_label],
